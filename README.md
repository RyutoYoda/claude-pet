...
- macOS 通知センターに通知
- 作業ログの表示・管理
- **Claude 利用量ウィジェット** — 今日のコスト・トークン数をヘッダーに常時表示。クリックで過去7日間の棒グラフ表示（30秒ごとに自動更新）
- ダークモード / ライトモード切替
- キャラクター画像のカスタマイズ
...
> **署名・Notarization 済み** — Apple Developer ID で署名・公証されているため、Gatekeeper の警告なしにインストールできます。
...
| ⚙ ボタン | 設定パネル |
| 承認パネルの ✅/❌ | 許可リクエストに回答 |
| ヘッダーの `$X.XX / YM` をクリック | 過去7日間の利用量グラフ表示/非表示 |
| グラフのバーにカーソルを乗せる | その日の日付・コスト・トークン数を表示 |
